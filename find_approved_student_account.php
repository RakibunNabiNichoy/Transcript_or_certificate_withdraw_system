...
{
    $sql="SELECT * FROM `approved_profiles` WHERE 1;";
    if (isset($_GET['search']))
    {
      $search = $_GET['search'];
      $sql = "SELECT * FROM `approved_profiles` WHERE  (`fullName` LIKE '%$search%' OR `department` LIKE '%$search%' OR `session` LIKE '%$search%' OR `email` LIKE '%$search%');";
...
<head>
    ...
    <title>Students who got approval</title>
...
<body>
  <h2 class="d-flex p-2 text-bg-primary justify-content-center ">Students who got approval</h2>
...
<?php
    while ($row = mysqli_fetch_row($result)) {
        $srl=$srl+1;
      //  echo $srl;
        $sql2="SELECT * FROM `student_profile` WHERE `roll`='$row[1]' AND `regNumber`='$row[2]';";
...
<div class="modal fade" id=<?php echo "s".$srl; ?> tabindex="-1"  aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered modal-dialog-scrollable">
    <div class="modal-content">
      ...
      <?php echo "Application Type : ".$row[14]."<br>";?>
      ...
      <?php echo "Payment Transaction Id : ".$row[13]."<br>";?>
      </div>
      <div class="modal-footer">
        <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Close</button>
        <!-- <a href="aprrovable.php?rg=<?php echo $row2[2];?>&rol=<?php echo $row2[1];?>&ind=<?php echo 0;?>&typ=<?php echo $row[2]; ?>"><button type="button" class="btn btn-primary">  accept</button></a>
        <a href="msgtest.php?rg=<?php echo $row2[2];?>&rol=<?php echo $row2[1];?>&ind=<?php echo 0;?>&typ=<?php echo $row[2]; ?>"><button type="button" class="btn btn-primary">  send message</button></a>
         -->
      </div>
    </div>
  </div>
