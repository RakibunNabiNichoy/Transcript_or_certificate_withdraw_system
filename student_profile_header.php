<nav class="navbar navbar-expand-lg bg-success">
    <div class="container">
        <a class="navbar-brand" href="student_login.php"><img src="varsity.png" alt="JKKNIU logo" style="width: 55px;"></a>
        <button class="navbar-toggler" type="button" data-bs-toggle="collapse" data-bs-target="#navbarNav" aria-controls="navbarNav" aria-expanded="false" aria-label="Toggle navigation" style="background-color: white;">
            <span class="navbar-toggler-icon"></span>
        </button>
        <div class="collapse navbar-collapse" id="navbarNav">
            <ul class="navbar-nav">
                <li class="nav-item">
                    <a class="nav-link " href="studend_profile.php">Profile Information</a>
                </li>
                <li class="nav-item">
                    <a class="nav-link" href="student_status.php">Status</a>
                </li>
                <li class="nav-item">
                    <a class="nav-link" href="find_your_withdraw_paper.php">Withdraw Paper</a>
                </li>
                <li class="nav-item">
                    <a class="nav-link" href="student_info_edit.php">Edit Profile</a>
                </li>
                <li class="nav-item">
                    <a class="nav-link" href="student_logout.php">Log Out</a>
                </li>


            </ul>
        </div>

    </div>
</nav>