...
{
    $sql="SELECT * FROM `student_status` WHERE `administrative_body`='No';";
    if (isset($_GET['search']))
    {
      $search = $_GET['search'];
      $sql = "SELECT * FROM `student_status` WHERE `administrative_body`='No' AND (TRIM(`fullName`)  LIKE '%$search%' OR `department` LIKE '%$search%' OR `session` LIKE '%$search%' OR `email` LIKE '%$search%');";
...
<head>
    ...
    <title>Administrative Admin</title>
...
<body>
  <h2 class="d-flex p-2 text-bg-primary justify-content-center ">Administrative Admin Profile</h2>
