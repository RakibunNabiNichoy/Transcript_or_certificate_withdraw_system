...
{
    $sql="SELECT * FROM `student_status` WHERE `cse_dept_body`='No';";
    if (isset($_GET['search']))
    {
      $search = $_GET['search'];
      $sql = "SELECT * FROM `student_status` WHERE `cse_dept_body`='No' AND (`fullName` LIKE '%$search%' OR `department` LIKE '%$search%' OR `session` LIKE '%$search%' OR `email` LIKE '%$search%');";
    }
...
<body>
  <h2 class="d-flex p-2 text-bg-primary justify-content-center ">CSE Department Admin Profile</h2>
  ...
  <div class="modal-dialog modal-dialog-centered modal-dialog-scrollable">
    <div class="modal-content">
      ...
      <div class="modal-footer">
        ...
        <a href="aprrovable.php?rg=<?php echo $row2[2];?>&rol=<?php echo $row2[1];?>&ind=<?php echo 7;?>&typ=<?php echo $row[2]; ?>"><button type="button" class="btn btn-primary">  accept</button></a>
        <a href="msgtest.php?rg=<?php echo $row2[2];?>&rol=<?php echo $row2[1];?>&ind=<?php echo 7;?>&typ=<?php echo $row[2]; ?>"><button type="button" class="btn btn-primary">  send message</button></a>
